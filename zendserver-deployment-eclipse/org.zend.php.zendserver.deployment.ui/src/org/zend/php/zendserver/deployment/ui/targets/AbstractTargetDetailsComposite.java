...
public abstract class AbstractTargetDetailsComposite {
	...
	public Job validate() {
		...
		job.addJobChangeListener(new JobChangeAdapter() {
			...
			public void done(IJobChangeEvent event) {
				...
				setErrorMessage(jobresult.getSeverity() == IStatus.OK ? null : message);
			}
		});
	 ...
	 * 
	 * @param defaultTarget target with default values.
	 */
	abstract public void setDefaultTargetSettings(IZendTarget defaultTarget);

	/**
	 * Returns essential data provided by user, that is necessary to create target.
	 * Called from UI thread, so should not block.
	 * 
	 ...
	 * @throws IOException
	 */
	abstract protected IZendTarget createTarget(String[] data) throws SdkException, IOException;
